		...
		trackedElements.forEach(function(node){
			if(node.querySelectorAll(traps).length===0){
				...
				$(node).popover({
					...
					placement:'auto top',
					content:function(){
						var href='https://ft-next-beacon-dashboard.herokuapp.com/graph?event_collection=cta&metric=count&group_by=meta.domPath&stacked_area=true&timeframe=this_7_days&interval=daily&title=Trackable%20element:%20'+$(node).data('trackable')+' (%25)';
